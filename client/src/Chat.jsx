...
// web-socket connection
  ...
  //============ jo online hai usko dikhane ke liye===============
    ...
    <div className={"flex h-screen chat-layout " + (theme === 'light' ? '' : 'bg-gray-800 text-white')}>
      <div className={(theme === 'light' ? 'bg-white' : 'bg-gray-700') + " chat-sidebar flex-col " + (selectedUserId ? 'hidden sm:flex' : 'flex')}>
        ...
        <div className="p-2 text-center flex items-center justify-center">
          <span className={"mr-2 text-sm flex items-center " + (theme === 'light' ? 'text-gray-600' : 'text-gray-300')}>
          ...
          <button
            onClick={toggleTheme}
            className="text-sm bg-blue-100 py-1 px-2 text-gray-500 border rounded-sm mr-2">
      ...
      <div className={"flex flex-col chat-main " + (theme === 'light' ? 'bg-blue-50' : 'bg-gray-600') + (selectedUserId ? ' flex' : ' hidden sm:flex')}>
        {!selectedUserId && (
        ...
        {!!selectedUserId && (
          <>
                          ...
                          {message.file.match(/\.(jpeg|jpg|gif|png)$/) ? (
                            <div className="rounded-md overflow-hidden">
                              <a target="_blank" href={axios.defaults.baseURL + '/uploads/' + message.file}>
                                <img src={axios.defaults.baseURL + '/uploads/' + message.file} alt="" className="max-w-xs max-h-48 object-cover"/>
                              </a>
                            </div>
                          ) : (
                            <div className="flex items-center gap-1 border-b mt-1 pt-1 border-gray-500">
            ...
            {/* Input Form */}
            <form className="flex gap-2 p-2 border-t" onSubmit={sendMessage}>
              <input type="text"
                     value={newMessageText}
                     onChange={ev => setNewMessageText(ev.target.value)}
                     placeholder="Type your message here"
                     className={(theme === 'light' ? 'bg-white' : 'bg-gray-700 text-white') + " flex-grow border rounded-sm p-2"}/>
              <label className={"p-2 cursor-pointer rounded-sm border " + (theme === 'light' ? 'bg-blue-200 text-gray-600 border-blue-200' : 'bg-gray-700 text-gray-300 border-gray-600')}>
